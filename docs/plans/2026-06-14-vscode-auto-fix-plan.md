...
**Goal:** Add a VS Code Quick Fix action named `Pi: Fix it` that sends the selected diagnostic context to Pi and automatically starts a Pi diagnostic-analysis turn.
...
**Outcome:** VS Code-family IDEs show `Pi: Fix it` under Quick Fix when the current location has error or warning diagnostics, and clicking it broadcasts a bounded diagnostic context payload to connected Pi clients.
...
- [ ] In `vscode/src/diagnostics.ts`, define constants `FIX_WITH_PI_COMMAND = "pi-x-ide.fixWithPiSuggest"`, `FIX_WITH_PI_TITLE = "Pi: Fix it"`, `DIAGNOSTIC_CONTEXT_RADIUS = 2`, and `MAX_SELECTED_TEXT_CHARS = 4000`.
...
- [ ] When eligible diagnostics exist, return exactly one `vscode.CodeAction` with title `Pi: Fix it`, kind `vscode.CodeActionKind.QuickFix`, `action.diagnostics` set to the filtered diagnostics, and `action.command.arguments` containing one normalized `DiagnosticFixRequestedParams` payload.
...
- [ ] If the server has zero connected clients, still broadcast the payload but also show `Pi x IDE: no Pi clients connected for Pi: Fix it.` so the user knows why no Pi turn starts.
...
- [ ] Document that `diagnostic_fix_requested` is sent by VS Code when the user selects `Pi: Fix it` from Quick Fix.
...
- [ ] In `README.md`, add the VS Code Quick Fix workflow to the VS Code / Cursor / Windsurf usage section: place the cursor on an error/warning, open Quick Fix, choose `Pi: Fix it`, and Pi starts a diagnostic-analysis turn.
...
- [ ] In `vscode/README.md`, add a feature bullet and usage paragraph for `Pi: Fix it` so the VSIX marketplace page reflects the new feature.
...
- [ ] While Pi is already streaming, trigger `Pi: Fix it` again and verify the request is queued as a follow-up instead of throwing.
- [ ] Disconnect Pi, trigger `Pi: Fix it`, and verify VS Code shows `Pi x IDE: no Pi clients connected for Pi: Fix it.` without crashing.
...
- Manual: Launch the VS Code Extension Development Host, trigger `Pi: Fix it` on an error/warning diagnostic, and confirm Pi receives and starts a diagnostic-analysis turn with the fixed template.
