...
Pi × IDE 通过三层配置：各 IDE 设置、Pi 侧环境变量，以及 `~/.pi/pi-x-ide/config.json` 中的顶层选项。

Pi 侧变量可设为真实环境变量 **或** 写入 `~/.pi/pi-x-ide/config.json` 的 `env`。真实环境变量优先级更高。
...
| 变量                            | 默认值       | 说明                                                                        |
| ------------------------------- | ------------ | --------------------------------------------------------------------------- |
| `PI_X_IDE_AUTO_INSTALL`         | `1`          | Pi 启动时自动安装 VS Code 扩展。设为 `0`/`false`/`off` 可禁用。             |
